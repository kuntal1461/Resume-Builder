...
export async function lookupUserProfileByEmail(
  email: string,
  ...
  const response = await fetch(
    `${apiBaseUrl}/auth/profile?email=${encodeURIComponent(trimmed)}`,
  );
  ...
  if (!response.ok) {
    ...
    throw new Error(
      detail || "Unable to resolve the user profile from the API.",
    );
  }
