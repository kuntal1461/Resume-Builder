...
import type {
  ...
  LikedJobSnapshot,
  StageKey,
} from "../../../components/workspace/job-tracking/types";
...
const ADDABLE_STAGES: StageKey[] = STAGE_ORDER.filter(
  (stage) => stage !== "Job Liked",
);
...
const buildLikedPipelineJobs = (snapshots: LikedJobSnapshot[]): PipelineJob[] =>
  snapshots.map((snapshot) => {
    const savedDate = snapshot.savedAt
...
export default function WorkspaceJobTrackingPage() {
  ...
  const [trackedJobsByStage, setTrackedJobsByStage] = useState<
    Partial<Record<StageKey, PipelineJob[]>>
  >({});
  ...
  const totalTracked = pipeline.reduce(
    (sum, column) => sum + column.jobs.length,
    0,
  );
    ...
    if (job.stage === "Interview") {
      ...
      if (job.interviewTime) {
        ...
        if (!Number.isNaN(hour) && !Number.isNaN(minute)) {
          ...
          formattedTime = temp.toLocaleTimeString([], {
            hour: "numeric",
            minute: "2-digit",
          });
        }
      ...
      createdJob.interviewDetails = detailParts.length
        ? detailParts.join(" · ")
        : undefined;
        ...
        <div className={styles.pageWrapper}>
          <section className={styles.jobTrackingHero}>
            <div className={styles.jobTrackingHeroContent}>
              ...
              <p>
                Keep every opportunity visible, get proactive reminders, and
                rally your job search rituals from a single control room.
              </p>
              <div
                className={styles.jobTrackingHighlights}
          ...
          <section className={styles.filtersBar} aria-label="Pipeline filters">
            <div className={styles.filterGroup}>
              ...
              <span
                className={`${styles.filterLabel} ${styles.filterLabelInline}`}
              >
                Sort by
              </span>
              <button type="button" className={styles.filterPill}>
            ...
            <div className={styles.filtersBarActions}>
              ...
              <button
                ...
                aria-label="Export tracker as CSV"
              >
                Export tracker
              </button>
            </div>
                    ...
                    {config.quickAction ? (
                      <div className={styles.columnActions}>
                        <button
                          ...
                          aria-label={
                            config.quickActionAria ?? config.quickAction
                          }
                        >
                          {config.quickAction}
                        </button>
                      ...
                      column.jobs.map((job) => (
                        <article key={job.id} className={styles.jobCard}>
                          <header className={styles.jobHeader}>
                            ...
                            {job.fit ? (
                              <span className={styles.jobFit}>{job.fit}</span>
                            ) : null}
                          </header>
                          ...
                          {job.interviewDetails ? (
                            <p className={styles.jobInterview}>
                              {job.interviewDetails}
                            </p>
                          ) : null}
                          {job.tags?.length ? (
                      ...
                      <div className={styles.emptyState}>
                        ...
                        <div>
                          ...
                          <p className={styles.emptyText}>
                            Add a role when you discover it or sync from your
