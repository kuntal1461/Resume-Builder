...
export default function OfferPackageAnalyzerPage() {
  ...
  const computed = useMemo(() => {
    return offers.map((o) => {
      ...
      const bonus = Math.max(
        0,
        Math.round((toNumber(o.bonusPct) / 100) * base),
      );
  ...
  const bestNetId = useMemo(() => {
    const best = computed.reduce<{ id: string; net: number } | null>(
      (accumulator, current) => {
  ...
  const bestPowerId = useMemo(() => {
    const best = computed.reduce<{ id: string; power: number } | null>(
      (accumulator, current) => {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Compare full compensation packages with taxes and cost of living."
        />
      </Head>
      <AppShell
        ...
        <div className={styles.page}>
          <header className={styles.header}>
            <div>
              ...
              <p>
                Enter offers to compare total compensation, estimated after-tax
                take-home, and cost-of-living adjusted purchasing power.
              </p>
            </div>
            <div className={styles.actions}>
              ...
              <button
                ...
                onClick={addOffer}
                aria-label="Add another offer to compare"
              >
                Add offer
              </button>
            </div>
            ...
            {computed.map((c) => (
              <article key={c.id} className={styles.resultCard}>
                ...
                <span className={styles.metricValue}>
                  {formatCurrency(c.total, currency)} total
                </span>
                ...
                <span
                  ...
                  style={{ color: c.id === bestNetId ? "#1f7f4c" : undefined }}
                >
                  {formatCurrency(c.net, currency)}{" "}
                  {c.id === bestNetId ? "(best net)" : ""}
                </span>
                <div className={styles.insights}>
                  <div className={styles.insightItem}>
                    ...
                    <span
                      style={{
                        fontWeight: c.id === bestPowerId ? (700 as const) : 500,
                      }}
                    >
                      {formatCurrency(c.purchasingPower, currency)}{" "}
                      {c.id === bestPowerId ? "(best CoL adj.)" : ""}
                    </span>
                  </div>
                </div>
              </article>
