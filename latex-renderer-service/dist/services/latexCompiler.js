...
const ensureLatexmkAvailable = () => {
  ...
  const result = (0, node_child_process_1.spawnSync)("latexmk", ["-v"], {
    stdio: "ignore",
  });
...
const runLatexmk = (workdir) => {
  return new Promise((resolve, reject) => {
    ...
    const child = (0, node_child_process_1.spawn)("latexmk", args, {
      cwd: workdir,
    });
    let log = "";
...
const compileLatexToPdf = async (latexSource) => {
  ...
  const tempDir = await (0, promises_1.mkdtemp)(
    node_path_1.default.join((0, node_os_1.tmpdir)(), "latex-render-"),
  );
