...
const UPLOAD_CHECKLIST = [
  {
    ...
    detail:
      "Remove custom packages that are not part of TeX Live 2023 to avoid build failures in the renderer.",
  },
  {
    title: "One resume per file",
    detail:
      "Keep each résumé in its own .tex for easier versioning and to align with template approvals.",
  },
  {
    title: "Placeholder tokens",
    detail:
      "Use {{candidate_name}} and {{role_title}} tokens so downstream automations can hydrate content.",
  },
];
...
export default function LatexUploadPage() {
  ...
  const [parentCategoryError, setParentCategoryError] = useState<string | null>(
    null,
  );
  const [isParentDropdownOpen, setIsParentDropdownOpen] = useState(false);
  const [isChildCategoryLoading, setIsChildCategoryLoading] = useState(false);
  const [childCategoryError, setChildCategoryError] = useState<string | null>(
    null,
  );
  ...
  const handleSave = async (targetStatus: "draft" | "published") => {
    ...
    if (
      latestSavedVersionLabel &&
      trimmedVersionLabel === latestSavedVersionLabel
    ) {
      setSaveError(
        "Update the version label (e.g., bump v1 to v1.1 or v2) before saving again.",
      );
      return;
    }
  ...
  useEffect(() => {
    ...
    const templateIdParam = Array.isArray(templateQuery)
      ? templateQuery[0]
      : templateQuery;
    if (!templateIdParam) {
    ...
    const loadTemplate = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${basePath}/api/templates/${numericTemplateId}`,
        );
        if (!response.ok) {
          const reason = await response.text();
          throw new Error(
            `Unable to load template (status ${response.status}): ${reason || "unknown error"}`,
          );
        }
        ...
        if (!("template_id" in data)) {
          const message =
            "error" in data && data.error
              ? data.error
              : "Unexpected response while loading template.";
        ...
        if (isMounted) {
          ...
          setChildCategories(
            data.child_category_slug ? [data.child_category_slug] : [],
          );
          setLatexSource(data.latex_source ?? "");
          const derivedVersionLabel =
            (data.version_label &&
              data.version_label.trim().length > 0 &&
              data.version_label) ||
  ...
  useEffect(() => {
    ...
    const loadParentCategories = async () => {
      ...
      try {
        const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
        const response = await fetch(
          `${basePath}/api/templates/parent-categories`,
        );
        if (!response.ok) {
          throw new Error(
            `Unable to load parent categories (status ${response.status}).`,
          );
        }
        const data = (await response.json()) as {
          success: boolean;
        ...
        if (isMounted) {
          ...
          setSelectedParentCategory((previous) => {
            if (
              previous &&
              data.categories.some((category) => category.slug === previous)
            ) {
              return previous;
            }
            return "";
  ...
  useEffect(() => {
    ...
    const loadChildCategories = async () => {
      ...
      const parent = parentCategories.find(
        (c) => c.slug === selectedParentCategory,
      );
      if (!parent) {
        setAvailableChildCategories([]);
        setChildCategories([]);
      ...
      try {
        const basePath = process.env.NEXT_PUBLIC_BASE_PATH ?? "";
        const response = await fetch(
          `${basePath}/api/templates/child-categories?parentId=${parent.id}`,
        );
        if (!response.ok) {
          throw new Error(
            `Unable to load child categories (status ${response.status}).`,
          );
        }
        const data = (await response.json()) as {
          success: boolean;
        ...
        if (isMounted) {
          ...
          setChildCategories((prev) =>
            prev.filter((slug) => data.categories.some((c) => c.slug === slug)),
          );
        }
      } catch (error) {
        if (isMounted) {
          const message =
            error instanceof Error
              ? error.message
              : "Unable to load child categories.";
  ...
  const selectedParentCategoryLabel =
    parentCategories.find(
      (category) => category.slug === selectedParentCategory,
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Upload LaTeX resume sources so admins can publish templates to the JobMatch pods."
        />
      </Head>
        ...
        <div className={styles.workspaceLayout}>
          ...
          <div className={styles.workspaceMain}>
            <section className={styles.sectionIntro}>
              ...
              <p>
                Drop LaTeX files, tweak the raw source, and confirm the rendered
                view before publishing to pods.
              </p>
            </section>
            ...
            <div className={styles.editorLayout}>
              <div className={styles.latexEditorCard}>
                  ...
                  <label>
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          aria-hidden="true"
                          className={styles.customSelectCaret}
                        >
                          ▾
                        </span>
                      </button>
                      {isParentDropdownOpen ? (
                        ...
                        >
                          {parentCategories.length === 0 ? (
                            <p className={styles.customSelectEmpty}>
                          ...
                          {parentCategories.map((category) => (
                            <button
                              ...
                              className={`${styles.customSelectOption} ${
                                selectedParentCategory === category.slug
                      ...
                      >
                        {(() => {
                          if (isChildCategoryLoading)
                            return <span>Loading child categories…</span>;
                          if (childCategoryError)
                            return <span>Child categories unavailable</span>;
                          if (availableChildCategories.length === 0)
                            return <span>No child categories</span>;
                          ...
                          if (selectedNames.length === 0)
                            return <span>Select child categories</span>;
                          ...
                          return (
                            <span className={styles.selectTriggerContent}>
                              ...
                              {extra > 0 ? (
                                <span className={styles.selectedBadgeMore}>
                                  +{extra}
                                </span>
                              ) : null}
                            </span>
                      ...
                      {isChildDropdownOpen ? (
                        ...
                        >
                          ...
                          {(() => {
                            ...
                            const list = query
                              ? availableChildCategories.filter((c) =>
                            ...
                            return (
                              <div className={styles.customSelectGrid}>
                                {list.map((option) => {
                                  const checked = childCategories.includes(
                                    option.slug,
                                  );
                                  return (
                                    <label
                                    ...
                                    >
                                      <input
                                        ...
                                        checked={checked}
                                        onChange={() =>
                                          toggleChildCategory(option.slug)
                                        }
                ...
                <label
                  className={styles.latexTextareaLabel}
                ...
                <div className={styles.latexTextareaGroup}>
                  <div
                    className={styles.latexLineNumbers}
                    aria-hidden="true"
                    ref={lineNumbersRef}
                  >
                ...
                <div className={styles.editorActions}>
                  ...
                  >
                    {isSaving && pendingSaveStatus === "draft"
                      ? "Saving…"
                      : "Save draft"}
                  </button>
                  <button
                    ...
                    disabled={isSaving || isTemplateLoading}
                  >
                    {isSaving && pendingSaveStatus === "published"
                      ? "Saving…"
                      : "Save"}
                  </button>
                </div>
              ...
              <div className={styles.previewPanel}>
                <div className={styles.previewToolbar}>
                  <div>
                    ...
                    <p>
                      Matches the PDF + HTML render pods see before publish.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div className={styles.previewPdfStage} aria-live="polite">
                  {isRendering ? (
                    ...
                    <div className={styles.previewPlaceholder}>
                      Click Render preview to view the PDF output.
                    </div>
                  )}
                </div>
            ...
            <section className={styles.sectionIntro}>
              ...
              <p>
                Every upload runs through these guardrails before surfacing in
                the workspace.
              </p>
            </section>
