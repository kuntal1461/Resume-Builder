...
const QUEUE_FILTERS = [
  {
    label: "All drafts",
    value: "all",
    description: "Everything waiting in queue",
  },
...
export default function DraftQueuePage() {
  ...
  const [copyResetTimeout, setCopyResetTimeout] =
    useState<NodeJS.Timeout | null>(null);
  const [lastRefreshed, setLastRefreshed] = useState<Date | null>(null);
  const [activeFilter, setActiveFilter] =
    useState<(typeof QUEUE_FILTERS)[number]["value"]>("all");
  const [versionOptions, setVersionOptions] = useState<
    Record<number, TemplateVersionRecord[]>
  >({});
        ...
        if (isMounted) {
          const message =
            loadError instanceof Error
              ? loadError.message
              : "Unable to load draft templates.";
          setError(message);
        }
      } finally {
        if (isMounted) {
  ...
  const metrics = useMemo(() => {
    ...
    const updatedRecently = drafts.filter((draft) =>
      wasUpdatedWithinHours(draft.last_update_time, 24),
    ).length;
    const staleDrafts = drafts.filter(
      (draft) => !wasUpdatedWithinHours(draft.last_update_time, 72),
    ).length;
    const assignedOwners = new Set(
      drafts
        .map(
          (draft) =>
            draft.owner_email?.toLowerCase() ||
            draft.owner_name?.toLowerCase() ||
            "",
        )
        .filter(Boolean),
    ).size;
    const unassignedDrafts = drafts.filter(
      (draft) => !draft.owner_email && !draft.owner_name,
    ).length;
    return {
      totalDrafts,
      updatedRecently,
    ...
    async (templateId: number) => {
      ...
      try {
        ...
        const response = await fetch(
          `${basePath}/api/templates/${templateId}/versions`,
          {
            headers: { Accept: "application/json" },
          },
        );
        ...
        setSelectedVersions((prev) => ({
          ...prev,
          [templateId]:
            data.versions && data.versions.length ? data.versions[0].id : null,
        }));
      } catch (fetchError) {
        const message =
  ...
  const queueListContent = useMemo(() => {
    if (isLoading) {
      return (
        <div className={styles.queueEmptyState}>Loading draft templates…</div>
      );
    }
    ...
    if (drafts.length === 0) {
      return (
        <div className={styles.queueEmptyState}>
          ...
          <p>
            Try publishing a template or check back after contributors upload
            LaTeX sources in the builder workflow.
          </p>
        </div>
      );
    }
    ...
    if (filteredDrafts.length === 0) {
      const filterLabel =
        QUEUE_FILTERS.find((chip) => chip.value === activeFilter)?.label ||
        "selected";
      return (
        <div className={styles.queueEmptyState}>
    ...
    return (
      <div className={styles.savedTemplatesGrid} aria-live="polite">
        {filteredDrafts.map((draft) => {
          ...
          const relativeUpdatedAt = formatRelativeUpdatedAt(
            draft.last_update_time,
          );
          const absoluteUpdatedAt = formatAbsoluteTimestamp(
            draft.last_update_time,
          );
          ...
          const computedVersionValue =
            selectedVersions[draft.id] ??
            (versions && versions.length ? versions[0].id : null);
          const versionSelectValue = computedVersionValue
            ? String(computedVersionValue)
            : "";
          ...
          return (
            <article
              key={draft.id}
              className={`${styles.savedTemplateCard} ${styles.queueCardCompact}`}
            >
              ...
              <div className={styles.savedTemplateMeta}>
                ...
                <div>
                  ...
                  <small className={styles.queueCardTimestampModern}>
                    {absoluteUpdatedAt}
                  </small>
                </div>
              </div>
              ...
              <div className={styles.savedTemplateActions}>
                <button
                  type="button"
                  onClick={() => handleReviewDraft(draft.id, router)}
                >
                  Review draft
                </button>
                <button
                  ...
                  className={`${styles.savedTemplateGhostButton} ${
                    copiedTemplateId === draft.id
                      ? styles.queueGhostButtonCopied
                      : ""
                  }`}
                  onClick={async () => {
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Track draft resume templates, blockers, and reviewer assignments."
        />
      </Head>
          ...
          <div className={styles.workspaceMain}>
            <section className={styles.savedTemplatesHero}>
              <div>
                ...
                <p>
                  This queue mirrors the workspace overview and surfaces the
                  same signals reviewers already trust. Share it with hiring
                  pods to keep drafts moving toward publish.
                </p>
            ...
            >
              <header className={styles.savedTemplatesPanelHeader}>
                <div>
                  ...
                  <p>
                    Prioritize review-ready drafts, hand off blockers, and keep
                    pods in sync.
                  </p>
                </div>
                <div
                  ...
                  role="tablist"
                  aria-label="Draft queue filters"
                >
...
function wasUpdatedWithinHours(
