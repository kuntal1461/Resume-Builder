...
const resolveOptionCode = (
  options: EnumOptionRecord[],
  ...
  "Nimbus AI Lab": [
    {
      ...
      description:
        "We are seeking a Senior ML Engineer to lead the development of our next-generation AI models. You will work on cutting-edge machine learning projects, collaborate with cross-functional teams, and mentor junior engineers.",
      requirements: [
    ...
    },
    {
      ...
      description:
        "Join our design team to create beautiful, intuitive user experiences for our AI-powered products. You will own the design process from research to final implementation.",
      requirements: [
    ...
    },
    {
      ...
      description:
        "Analyze complex datasets to drive business insights and build predictive models. Work closely with product and engineering teams to implement data-driven solutions.",
      requirements: [
    ...
    },
    {
      ...
      description:
        "Build responsive, performant web applications using modern JavaScript frameworks. Collaborate with designers and backend engineers to deliver exceptional user experiences.",
      requirements: [
        "React/Next.js expert",
        "TypeScript proficiency",
        "3+ years frontend dev",
        "CSS/Tailwind skills",
      ],
      isRejected: false,
    },
    {
      ...
      description:
        "Manage and scale our cloud infrastructure, implement CI/CD pipelines, and ensure high availability of our services. Work with engineering teams to optimize deployment processes.",
      requirements: [
        "AWS/GCP experience",
        "Kubernetes, Docker",
  ...
  "Skyline Analytics": [
    {
      ...
      description:
        "Build and maintain data pipelines and analytics infrastructure. Transform raw data into actionable insights for business stakeholders.",
      requirements: [
        "SQL mastery",
        "dbt experience",
        "Data warehousing",
    ...
    },
    {
      ...
      postedDate: "1 week ago",
      description:
        "Partner with business leaders to identify opportunities, analyze data, and drive strategic decisions. Create dashboards and reports to track key metrics.",
      requirements: [
        "3+ years BA experience",
        "Excel/Tableau expert",
    ...
    },
    {
      ...
      postedDate: "2 days ago",
      description:
        "Design and build scalable data infrastructure to support analytics and ML workloads. Optimize data pipelines for performance and reliability.",
      requirements: [
        "Spark, Airflow",
        "Cloud platforms",
        "ETL/ELT expertise",
  ...
  "CivicTech Labs": [
    {
      ...
      postedDate: "3 days ago",
      description:
        "Develop civic technology solutions that improve government services and citizen engagement. Work on impactful projects that serve the public good.",
      requirements: [
    ...
    },
    {
      ...
      postedDate: "1 week ago",
      description:
        "Conduct user research to understand citizen needs and inform product decisions. Plan and execute usability studies, interviews, and surveys.",
      requirements: [
        "UX research methods",
...
export default function AdminJobTrackerPage({
  ...
  const cadenceOptions = scrapeCadences.length
    ? scrapeCadences
    : FALLBACK_CADENCE_OPTIONS;
  const scrapeTypeOptions = scrapeTypes.length
    ? scrapeTypes
    : FALLBACK_SCRAPE_TYPE_OPTIONS;
  ...
  const [linkRows, setLinkRows] = useState<LinkRow[]>(() => [
    createLinkRow("row-0", defaultSelections),
  ]);
  ...
  const updateLinkRow = <K extends keyof LinkRow>(
    index: number,
    field: K,
    value: LinkRow[K],
  ) => {
  ...
  const addLinkRow = () => {
    ...
    setLinkRows((previous) => [
      ...previous,
      createLinkRow(nextId, defaultSelections),
    ]);
  };
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    for (let index = 0; index < linkRows.length; index += 1) {
      ...
      if (!row.company.trim()) {
        setBanner({
          type: "error",
          message: `Add a company name on row ${index + 1}.`,
        });
        return;
      }
      if (!isValidUrl(row.url.trim())) {
        setBanner({
          type: "error",
          message: `Enter a valid job board URL on row ${index + 1}.`,
        });
        return;
      }
      if (row.scrapeType === "API") {
        if (!row.apiEndpoint.trim()) {
          setBanner({
            type: "error",
            message: `Provide an API endpoint on row ${index + 1}.`,
          });
          return;
        }
        if (!isValidUrl(row.apiEndpoint.trim())) {
          setBanner({
            type: "error",
            message: `Enter a valid API endpoint URL on row ${index + 1}.`,
          });
          return;
        }
        if (!row.apiKey.trim()) {
          setBanner({
            type: "error",
            message: `Provide an API key on row ${index + 1}.`,
          });
          return;
        }
      }
    ...
    const jobSourcePayload: JobSourceQueueRequest = {
      entries: linkRows.map((row) => ({
        ...
        scrapeTypeId:
          resolveOptionCode(scrapeTypeOptions, row.scrapeType) ?? null,
        cadence: row.cadence,
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Unable to queue job sources. Try again.";
    ...
    const newEntries = responseEntries.length
      ? responseEntries
      : fallbackEntries;
        ...
        <div className={layoutStyles.workspaceLayout}>
          <aside className={layoutStyles.workspaceSidebar}>
            <Link
              ...
              className={layoutStyles.sidebarBrandLink}
            ...
            <section
              className={layoutStyles.sidebarProfileCard}
              aria-labelledby="profile-card-title"
            >
              <div className={layoutStyles.sidebarProfileHeader}>
                <span
                  className={layoutStyles.sidebarProfileAvatar}
                  aria-hidden="true"
                >
                ...
                <div className={layoutStyles.sidebarProfileMeta}>
                  <span
                    className={layoutStyles.sidebarProfileName}
                    id="profile-card-title"
                  >
                    {sidebarProfile.name}
                  </span>
                  <span className={layoutStyles.sidebarProfileTagline}>
                    {sidebarProfile.tagline}
                  </span>
                  <span className={layoutStyles.sidebarProfileEmail}>
          ...
          <div className={layoutStyles.workspaceMain}>
            <section className={styles.hero}>
              ...
              <p>
                Drop approved job URLs, decide how often we scrape them, and
                keep a single view on ingestion status. Every source runs
                through compliance before it lands in the job-matching graph.
              </p>
            ...
            <section className={styles.formPanel}>
              <header className={styles.panelHeader}>
                ...
                <p className={styles.panelHint}>
                  We&apos;ll validate schema, robots.txt, and attach it to the
              ...
              <form className={styles.form} onSubmit={handleSubmit}>
                {banner ? (
                  <div
                    className={`${styles.banner} ${banner.type === "error" ? styles.bannerError : styles.bannerSuccess}`}
                  >
                    {banner.message}
                  </div>
                ) : null}
                ...
                <p className={styles.complianceCopy}>
                  By submitting you confirm the source allows programmatic
                  access and that contracts cover candidate redistribution.
                </p>
              ...
              {groupedResults.length ? (
                <div className={styles.queuePanel}>
                  <header className={styles.panelHeader}>
                    ...
                    <p className={styles.panelHint}>
                      Expand a company to see every URL we are monitoring and
                      whether scraping is working or pending.
                    </p>
                  </header>
                    ...
                    {groupedResults.map((group) => {
                      ...
                      return (
                        <li key={group.company} className={styles.queueCard}>
                          ...
                          >
                            <div>
                              ...
                              <span className={styles.companyMeta}>
                                {group.totalSources} source
                                {group.totalSources === 1 ? "" : "s"}
                              </span>
                            </div>
                            <div className={styles.companyStats}>
                              <span
                                ...
                                onClick={(e) =>
                                  toggleJobsView(group.company, e)
                                }
                                style={{ cursor: "pointer" }}
                              >
                                {group.totalJobs} job
                                {group.totalJobs === 1 ? "" : "s"} scraped
                              </span>
                              <span
                                className={styles.companyToggleIcon}
                                aria-hidden="true"
                              >
                                {isExpanded ? "−" : "+"}
                              </span>
                          ...
                          {expandedJobs[group.company] &&
                          jobs[group.company] ? (
                            <div className={styles.jobsContainer}>
                              {jobs[group.company].map((job) => {
                                const isJobExpanded = Boolean(
                              ...
                              {group.entries.map((entry) => (
                                ...
                                >
                                  <div className={styles.queueCardHeader}>
                                    ...
                                    <span className={styles.cadencePill}>
                                      {entry.cadence}
                                    </span>
                                  </div>
...
> = async () => {
  try {
    const { fetchJobSourceMetadata } =
      await import("../../lib/server/jobSources");
